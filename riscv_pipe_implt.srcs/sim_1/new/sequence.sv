class rand_seq extends uvm_sequence #(seq_item);
  ...
  task body() ;
    // Create the item using UVM factory 	
    rand_item = seq_item::type_id::create("rand_item");
    
    // Request permission to send item to driver
    start_item(rand_item); 
    
    // Randomizing the items with constraints
    ...
    // Send item to driver for execution
...
class addi_seq extends uvm_sequence #(seq_item);
  ...
  task body() ; 	
    ...
    assert(addi_item.randomize() with {
           ...
           addi_item.instrf[19:15] != 0; 
           addi_item.instrf[11:7]  != 0; 
